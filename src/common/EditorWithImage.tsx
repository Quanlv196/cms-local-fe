...
const InputTitleTop = (props: rootProps) => {
  ...
  const handleImageUpload = (file: any) => {
    return new Promise((resolve, reject) => {
      // Simulate a delay for image upload (replace this with actual upload logic)
      setTimeout(() => {
  ...
  const handleUpload = async (file: any) => {
    ...
    const responeImage: any = await APIClient.POST('/storage/upload', dtaForm)
    if (responeImage && responeImage.response) {
      url = responeImage.response
    }
      ...
      <InputGroup>
        {typeEdit === 'sunEditor' ?
          <Editor
            ...
            toolbar={{
              image: {
                uploadCallback: (file: any) => handleUpload(file),
                previewImage: true,
                urlEnabled: false
              },
            }}
